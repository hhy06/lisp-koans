...
(define-test gethash
  ...
  (let ((cube-roots (make-hash-table)))
    ...
    (setf (gethash -27 cube-roots) -3)
    (assert-equal -3 (gethash -27 cube-roots))
...
(define-test hash-table-equality
  ...
  (let ((hash-table-1 (make-hash-table :test #'equal))
    ...
    (true-or-false? NIL (equal hash-table-1 hash-table-2)) ;; !! this is implementation dependent. hhy.
